...
public class CartController {
	...
	public String UpdateCart(HttpServletRequest request, Model model) throws Exception {
		...
		for (Cart o : listCart) {
		    ...
		    //try {
		        ...
		        if (count <= 0) {
		        	session.setAttribute("NoSignIn", "Số lượng ko được nhỏ hơn 0");
		        	return "redirect:/cart";
		        }
		        if(count >1000) {
		        	session.setAttribute("NoSignIn", "Số lượng ko được lớn hơn hơn 9999");
		        	return "redirect:/cart";
		        }
	...
	public String AddToCartPost(@ModelAttribute("product_id") int product_id, @ModelAttribute("count") String a,
	    ...
	    if (user == null) {
	        ...
	        if (count <= 0) { // Kiểm tra giá trị của count
	            session.setAttribute("AddToCartErr", "Số lượng phải là số dương");
	            return "redirect:" + referer;
	        }
	        if (count > 9999) { // Kiểm tra giá trị của count
	            session.setAttribute("AddToCartErr", "Số lượng ko vượt quá 9999");
	            return "redirect:" + referer;
	        }
