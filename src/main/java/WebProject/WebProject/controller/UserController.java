	...
	public String SignUp(@ModelAttribute("username") String id, @ModelAttribute("your_email") String email,
		...
		if (user == null) {
			if (password.length() < 6) {
				model.addAttribute("errorSignUp", "Mật khẩu phải chứa ít nhất 6 ký tự!");
				return "signup";
			}
			if (!password.equals(comfirm_password)) {
				model.addAttribute("errorSignUp", "Mật khẩu xác nhận không khớp!");
				return "signup";
			}
			// kiểm tra tính hợp lệ của địa chỉ email ở đây
